...
public class AccessTokenConverter extends JwtAccessTokenConverter {
    ...
    public AccessTokenConverter(UserDetailsService userDetailsService) {
        // 设置签名私钥
    ...
    public OAuth2AccessToken enhance(OAuth2AccessToken accessToken, OAuth2Authentication authentication) {
        final Authentication user = authentication.getUserAuthentication();
        final String[] authorities =
                user.getAuthorities().stream()
                                     .map(GrantedAuthority::getAuthority)
                                     .toArray(String[]::new);

        final Map<String, Object> payLoad = Maps.newHashMapWithExpectedSize(2);
